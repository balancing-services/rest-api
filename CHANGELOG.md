...
### Fixed
- The Python client's example scripts and README now target the v2 API base URL (`https://api.balancing.services/v2`). They still pointed at `/v1`, so the 2.0 client — which speaks the v2 wire format — raised `KeyError: 'nextUpdatedSince'` on the first response. Documentation links now point at the v2 docs as well. No change to the client package itself
