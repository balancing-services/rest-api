...
from ...models.imbalance_price_history_response import ImbalancePriceHistoryResponse
...
) -> ImbalancePriceHistoryResponse | Problem | None:
    if response.status_code == 200:
        response_200 = ImbalancePriceHistoryResponse.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[ImbalancePriceHistoryResponse | Problem]:
    return Response(
...
def sync_detailed(
    *,
    ...
    """Get the revision history of the imbalance prices for an area

     **Experimental** - This endpoint is under active development and may change without notice.

    Returns how the imbalance prices on `/imbalance/prices` reached their current values: the
    sequence of values held for a period, each stamped with the `observedAt` it was recorded at.
    The response is the `/imbalance/prices` shape with `observedAt` added to each entry of
    `prices`, which carries the revision entries, ascending by `observedAt`.
...
def sync(
    *,
    ...
    Returns:
        ImbalancePriceHistoryResponse | Problem
    """
