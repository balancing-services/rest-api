...
from ...models.balancing_energy_price_history_response import (
...
def _get_kwargs(
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/balancing/energy/prices/history",
        "params": params,
    }
...
) -> BalancingEnergyPriceHistoryResponse | Problem | None:
    if response.status_code == 200:
        response_200 = BalancingEnergyPriceHistoryResponse.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[BalancingEnergyPriceHistoryResponse | Problem]:
    return Response(
...
def sync_detailed(
    *,
    ...
    """Get the revision history of the balancing energy prices for an area

     **Experimental** - This endpoint is under active development and may change without notice.

    Returns how the balancing energy prices on `/balancing/energy/prices` reached their current
    values: the sequence of values held for a period, each stamped with the `observedAt` it was
    recorded at. The response is the `/balancing/energy/prices` shape with `observedAt` added to
    each entry of `prices`, which carries the revision entries, ascending by `observedAt`.
...
def sync(
    *,
    ...
    Returns:
        BalancingEnergyPriceHistoryResponse | Problem
    """
